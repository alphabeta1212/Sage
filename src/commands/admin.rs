...
#[group]
#[owners_only]
#[commands(everyone, set_quote_channel, set_mod_channel)]
#[only_in(guild)]
struct Admin;
...
#[command]
async fn set_mod_channel(ctx: &Context, msg: &Message) -> CommandResult {
    let subscriber = msg.channel_id.0;
    let mut db = Database::new();
    let db_uri = env::var("MONGO_URI").unwrap();
    let _ = db.make_connection(db_uri).await;
    if let Err(_) = db
        .add_subscriber_to("mod_channel".to_string(), subscriber)
        .await
    {
